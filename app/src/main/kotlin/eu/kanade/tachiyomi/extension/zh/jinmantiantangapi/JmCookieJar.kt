...
class JmCookieJar(private val preferences: SharedPreferences) : CookieJar {
    ...
    fun getCookies(host: String): List<Cookie> {
        return cookieStore[host]?.filter { !it.expiresAt.isExpired() } ?: emptyList()
    }
    ...
    fun getCookieString(host: String): String {
        return getCookies(host).joinToString("; ") { "${it.name}=${it.value}" }
    }
     ...
     * @param cookieString Cookie 字符串，格式："name1=value1; name2=value2"
     */
    private fun parseCookieString(host: String, cookieString: String): List<Cookie> {
